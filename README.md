...
| ENHANCED_DISPLAY_FIELDS | Per-collection configuration of enhanced metadata fields to display in the Enhanced Details modal. Uses specific STAC field names organized by category: Project ("collection", "mission", "constellation"), Vendor ("platform", "instruments"), Product Type ("s2:product_type", "landsat:correction"), Time ("datetime", "created"), Spatial ("proj:epsg", "bbox"), Grid Systems ("mgrs:utm_zone", "landsat:wrs_path"), Quality ("eo:cloud_cover"), and Instrument ("sar:instrument_mode"). If not configured for a collection, all available fields are shown. Example in [config.example.json](config_helper/config.example.json). | Optional |
