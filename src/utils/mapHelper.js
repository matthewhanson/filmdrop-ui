...
function findReplaceTitilerUrl(featureURL) {
  ...
  const replaceTitilerURL = store.getState().mainSlice.appConfig.TILER_SETTINGS?.URL_SUBST ?? false
  if (replaceTitilerURL === true) {
    const findStr = store.getState().mainSlice.appConfig.TILER_SETTINGS.URL_SUBST_FIND
    const replaceStr = store.getState().mainSlice.appConfig.TILER_SETTINGS.URL_SUBST_REPLACE
    ...
    if (findStr === undefined || replaceStr === undefined) {
      ...
      console.warn('[TILER_SETTINGS.URL_SUBST] URL_SUBST_FIND and URL_SUBST_REPLACE must be strings. Skipping substitution.')
    } else {
      // replace can fail
      try {
